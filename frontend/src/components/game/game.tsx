...
import { Participant, ResultModal } from './components/components';
...
import { IParticipant, IUser, IUserAction } from 'common/interfaces/interfaces';
import { SocketContext } from 'socket/socket';
import { gameApi, userApi } from 'services/services';
...
export const Game: React.FC = () => {
  ...
  const onAddParticipant = (participant: Omit<IUser, 'email'>): void => {
  ...
  const handleParticipantEndedGame = (userId: string): void => {
    dispatch(gameActions.toggleIsReady(userId));
    dispatch(
  ...
  const handleToggleIsReady = (): void => {
    if (currentParticipant) {
      if (!text) {
        dispatch(gameActions.loadText(roomId as string));
      }
